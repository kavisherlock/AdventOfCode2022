...
vector<unsigned long long> splitAsInts(string s, string delimiter) {
  ...
  public:
    ...
    void performOperation() {
      for (int i = 0; i < items.size(); i += 1) {
        unsigned long long item = items[i];
...
int main () {
  ...
  u_long divisorProduct = 1;
  for (Monke monke : monkes) {
    divisorProduct *= monke.testDivisor;
  }

  for (int n = 0; n < 10000; n += 1) {
    for (int i = 0; i < nMonkes; i += 1) {
      monkes[i].performOperation();
      vector<int> nextMonkes = monkes[i].getNextMonkes(divisorProduct);
