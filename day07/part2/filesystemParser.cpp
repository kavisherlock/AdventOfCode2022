...
int getSmallestLargeSize(Item *item, int minSize) {
  int min = -1;
  if (item->size >= minSize && item->type == "DIR") {
    min = item->size;
  } else {
    return -1;
  }
  ...
  for (Item* child : item->children) {
    if (child->type == "DIR") {
      int size = getSmallestLargeSize(child, minSize);
      if ((size < min || min < 0) && size >= minSize) {
        min = size;
      }
    }
  }
...
int main () {
  ...
  cout << getSmallestLargeSize(root, 30000000 - (70000000 - root->size)) << endl;
}
